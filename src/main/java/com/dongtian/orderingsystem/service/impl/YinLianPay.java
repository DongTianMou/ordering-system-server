...
import com.dongtian.orderingsystem.sdk.DemoBase;
import com.dongtian.orderingsystem.sdk.LogUtil;
import com.dongtian.orderingsystem.sdk.SDKConfig;
import com.dongtian.orderingsystem.service.PayAdaptService;
...
public class YinLianPay implements PayAdaptService {
    ...
    public String pay(PaymentInfo paymentInfo, PaymentType paymentType) {
        return setPay(paymentInfo, paymentType);
    }

    @Override
    public String returnMoney(PaymentInfo paymentInfo, PaymentType paymentType) {
        return setReturnMoney(paymentInfo,paymentType);
    }
    private String setReturnMoney(PaymentInfo paymentInfo, PaymentType paymentType) {
        ...
        /*商户接入参数***/
        data.put("merId", paymentType.getMerchantId());//商户号码，请改成自己申请的商户号或者open上注册得来的777商户号测试
        data.put("accessType", "0");//接入类型，商户接入固定填0，不需修改
        data.put("orderId", paymentInfo.getOrderId());//商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则，重新产生，不同于原消费
        data.put("txnTime", DemoBase.getCurrentTime());//订单发送时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
        data.put("currencyCode", "156");//交易币种（境内商户一般是156 人民币）
        data.put("txnAmt", paymentInfo.getPrice().toString()); //****退货金额，单位分，不要带小数点。退货金额小于等于原消费金额，当小于的时候可以多次退货至退货累计金额等于原消费金额
        //后台通知地址
        data.put("backUrl", DemoBase.backUrl);
        //****原消费交易返回的的queryId，可以从消费交易后台通知接口中或者交易状态查询接口中获取
        data.put("origQryId", paymentInfo.getQryId());
    ...
    private String setPay(PaymentInfo paymentInfo, PaymentType paymentType) {
        ...
        requestData.put("riskRateInfo", "{商品名称}");

         //前台通知地址 （需设置为外网能访问 http https均可），支付成功后的页面 点击“返回商户”按钮的时候将异步通知报文post到该地址
        //如果想要实现过几秒中自动跳转回商户页面权限，需联系银联业务申请开通自动返回商户权限
        //异步通知参数详见open.unionpay.com帮助中心 下载  产品接口规范  网关支付产品接口规范 消费交易 商户通知
        requestData.put("frontUrl", paymentType.getFrontUrl()+paymentInfo.getOrderId());
